...
function hideLine(id){
    ...
    document.querySelectorAll('#inv'+id).forEach(ele => ele.style.display = (HiddenLines.checked)?'none':'table-row');
}
...
function hideOrShowLines(){
    if(HiddenLines.checked) {
        ...
        document.querySelectorAll('[id^=inv]').forEach(ele => {
            let id = (ele.id).replace('inv','');
            if(document.querySelector('#linha'+id).style.display == 'none') ele.style.display = 'table-row';
        });
    }
}